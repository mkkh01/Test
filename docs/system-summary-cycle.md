...
بعد أول رفض لتوقيع TxID، يظهر للعميل نموذج evidence باللغة الإنجليزية يجمع التوقيع مرة أخرى، وعنوان المرسل، وعنوان الاستلام، والمبلغ، ووقت التحويل، والنص أو الصورة الاختيارية. هذه البيانات المعلنة تُحفظ للتدقيق فقط؛ لا تُعتبر الصورة أو النص أو الوقت دليلًا مستقلًا على وصول المال. إذا وُجد TxID صالح، يعيد الخادم الاستعلام مباشرة من Solana RPC ويعتمد فقط على بيانات البلوكشين: الشبكة، عنوان الاستلام، USDT-SPL mint، مبلغ لا يقل عن الفاتورة، نجاح المعاملة، `finalized`، وعدم استعمال TxID في فاتورة أخرى.

شرط الأربع دقائق هو **شرط أمان إضافي للإطلاق الآلي**: يقارن `blockTime` الحقيقي في البلوكشين بوقت إنشاء الفاتورة. إذا خرجت المعاملة عن النافذة، تُحفظ كـ`outside_time_window` وتنتقل إلى `manual_review` بدل قبولها تلقائيًا أو رفضها اعتمادًا على وقت أدخله العميل. ولا يطلق النظام رابط ZIP إلا بعد تأكيد البلوكشين والمعاملة الذرية التي تحدّث الدفع والفاتورة وتنشئ token التنزيل.

أضيفت أعمدة التفاصيل ونتيجة التحقق عبر `supabase/migrations/008_payment_evidence_details.sql`، مع إبقاء جدول evidence خاصًا بـ`service_role` فقط. كما تم تعديل عامل إعادة فحص المدفوعات حتى يتخطى الطلبات الموجودة في `manual_review` ولا يتجاوز هذه السياسة.
...
يستخدم `discovery-worker.js` مصادر عامة فقط: Hacker News وBluesky وDEV Community وStack Exchange API العام، مع Stack Exchange المتخصص (Freelancing وProject Management وGraphic Design وWebmasters)، ومنتديات Discourse العامة (Webflow وGhost افتراضيًا)، وReddit OAuth وX Recent Search وGitHub Issues وBrave Search وGoogle News RSS كخيارات إضافية. Reddit وX وBrave لا تعمل إلا بعد إضافة بيانات الاعتماد الاختيارية إلى Render؛ Stack Exchange المتخصص ومنتديات Discourse وGitHub Issues تعمل عبر endpoints عامة، وGoogle News RSS هو مسار قراءة عام محدود لا يعتمد على كشط صفحة نتائج Google. العناصر تُحفظ في `source_items` مع deduplication، ثم تُنشأ وظائف تحليل في `jobs`. يقوم `lead-analysis-worker.js` بإرسال النص العام إلى Gemini ويخزن النتيجة في `leads` و`lead_analyses` و`outreach_messages`.
...
| مسار evidence بعد أول رفض | مطبق ومختبر آليًا؛ لا يعتمد على صورة أو بيانات العميل وحدها | يجب اختبار شراء Mainnet حقيقي منفصلًا وبموافقة صريحة |
